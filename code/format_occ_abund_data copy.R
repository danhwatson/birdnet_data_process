...
acoustic_dat <- read_csv("data/count_data_24_bacs_phenology.csv")
# load ARU timeline
...
acoustic_dat <- acoustic_dat %>%
  filter(date >= '2024-03-10' & date <= '2024-09-14')
...
#write 
write_csv(y.long, "data/phenology_24_bacs.csv")
...
#veg.covs <- veg %>% 
#  select(site, treatment, shrub_cover, grass_cover, forb_cover, bacsfer_cover, shrub_height, grass_height, forb_height, veg_cover_overall, veg_height_overall, veg_diversity_overall, ) %>%
#  arrange(site)

# Select and arrange ALL vegetation covariates
veg.covs <- veg %>% 
  select(site, treatment, shrub_cover, grass_cover, forb_cover, bacsfer_cover, shrub_height, grass_height, forb_height, bacsfer_height, veg_cover_overall, veg_height_overall, veg_diversity_overall, bluestem_cover, pine_cover, broadleaf_shrub_cover, forb_other_cover, grass_other_cover, non_native_cover, forb_vine_cover, fern_cover, saw_palmetto_cover, broadleaf_vine_cover, wiregrass_cover, bluestem_height, pine_height, broadleaf_shrub_height, forb_other_height, grass_other_height, non_native_height, forb_other_height, fern_height, saw_palmetto_height, broadleaf_vine_height, wiregrass_height) %>%
  arrange(site)
...
#abund.covs <- veg.covs %>%
#  select(treatment, shrub_cover, grass_cover, forb_cover, bacsfer_cover, shrub_height, grass_height, #forb_height, veg_cover_overall, veg_height_overall, veg_diversity_overall)

# Select all those veg covariates
abund.covs <- veg.covs %>%
  select(treatment, shrub_cover, grass_cover, forb_cover, bacsfer_cover, shrub_height, grass_height, forb_height, bacsfer_height, veg_cover_overall, veg_height_overall, veg_diversity_overall, bluestem_cover, pine_cover, broadleaf_shrub_cover, forb_other_cover, grass_other_cover, non_native_cover, forb_vine_cover, fern_cover, saw_palmetto_cover, broadleaf_vine_cover, wiregrass_cover, bluestem_height, pine_height, broadleaf_shrub_height, forb_other_height, grass_other_height, non_native_height, forb_other_height, fern_height, saw_palmetto_height, broadleaf_vine_height, wiregrass_height)
...
abund_data_bacs <- list(y = y_flat, abund.covs = abund.covs, det.covs = det.covs, coords = coords_matrix)
...
# Save as data object 
save(abund_data_bacs, file = "data/abundance_data/abundance_24_bacs_phenology.RData")
