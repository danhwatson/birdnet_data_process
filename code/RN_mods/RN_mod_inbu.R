...
#saveRDS(rn_model_t_inbu, file = "data/rn_model_t_inbu.rds") 
...
#fitstats_inbu <- function(rn_model_t_inbu) {
#  observed <- getY(rn_model_t_inbu@data)
#  expected <- fitted(rn_model_t_inbu)
#  resids <- residuals(rn_model_t_inbu)
...
#gof <- mb.gof.test(rn_model_t_inbu, nsim=100, c.hat.est=TRUE, model.type="RN")

#parametric bootstrapping 
# Assuming 'mod' is your fitted model
#pb_inbu <- parboot(rn_model_t_inbu, fitstats_inbu, nsim=100)
#print(pb_inbu)
...
grass_range <- seq(min(umf_inbu@siteCovs$grass_cover), max(umf_inbu@siteCovs$grass_cover), length.out = 100)
...
predictions_grass <- predict(rn_model_t_inbu, newdata_grass, type = "state")
...
ggplot(plot_data_grass, aes(x = grass_cover, y = predicted_state, color = treatment)) +
  ...
  labs(x = "Grass Cover", y = "Predicted Abundance", title = "Northern Bobwhite (inbu) -- Predicted Response to Grass Cover across Treatments") +
  theme_minimal()
...
predictions_grass <- predict(rn_model_inbu, newdata_grass, type = "state", se.fit = TRUE)
...
ggplot(newdata_grass, aes(x = grass_cover, y = predicted_state)) +
  ...
  labs(x = "Percent Grass Cover", y = "Relative Abundance per Site", title = "Northern Bobwhite (inbu) -- Predicted Response to Grass Cover") +
  theme_classic() +
  theme(
