...
summary(rn_model_7_cwwi)

#Treatment + Shrub Cover
rn_model_8_cwwi <- occuRN( 
  formula = ~ I(scale(day)^2) + scale(day) + scale(wind_speed) + scale(precipitation) 
...
#AICc comparison
models_list_cwwi_post_hoc <- list(rn_model_0_cwwi, #Treatment + Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height
                                  rn_model_1_cwwi, #Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height
                                  rn_model_2_cwwi, #Treatment
                                  rn_model_3_cwwi, #Treatment * Shrub Cover * Grass Cover
                                  rn_model_4_cwwi, #Treatment + Shrub Cover + Grass Cover
                                  rn_model_5_cwwi, #Treatment * Grass Cover
                                  rn_model_6_cwwi, #Treatment + Grass Cover
                                  rn_model_7_cwwi, #Treatment * Shrub Cover
                                  rn_model_8_cwwi, #Treatment + Shrub Cover
                                  rn_model_9_cwwi, #Shrub Cover * Grass Cover
                                  rn_model_10_cwwi, #Shrub Cover + Grass Cover
                                  rn_model_11_cwwi) #Null

#model names use descriptive 
model_names_cwwi_post_hoc <- c("Treatment + Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height", 
                               "Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height", 
                               "Treatment", 
                               "Treatment * Shrub Cover * Grass Cover", 
                               "Treatment + Shrub Cover + Grass Cover", 
...
aicc_table_cwwi_post_hoc <- aictab(cand.set = models_list_cwwi_post_hoc, modnames = model_names_cwwi_post_hoc)

print(aicc_table_cwwi_post_hoc)

# Load the master AICc table
aicc_table <- read.csv("data/aicc_table_posthoc.csv")

# Add species name to the table
aicc_table_cwwi_post_hoc$species_code <- "Chuck-will's-widow"

# Add the AICc table to the master table
aicc_table_master <- rbind(aicc_table, aicc_table_cwwi_post_hoc)

# Save the AICc table to csv
write.csv(aicc_table_master, "data/aicc_table_posthoc.csv", row.names = FALSE)

#vif for top model
vif_table_state_cwwi <- vif(rn_model_6_cwwi, type = "state")
vif_table_det_cwwi <- vif(rn_model_6_cwwi, type = "det")
