...
#abund_data_coye$abund.covs$treatment[abund_data_coye$abund.covs$treatment == "young_savanna"] <- "a_young_savanna"
...
summary(rn_model_7_coye)

#Treatment + Shrub Cover
rn_model_8_coye <- occuRN( 
  formula = ~ I(scale(day)^2) + scale(day) + scale(wind_speed) + scale(precipitation) 
...
#AICc comparison
models_list_coye_post_hoc <- list(rn_model_0_coye, #Treatment + Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height
                                  rn_model_1_coye, #Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height
                                  rn_model_2_coye, #Treatment
                                  rn_model_3_coye, #Treatment * Shrub Cover * Grass Cover
                                  rn_model_4_coye, #Treatment + Shrub Cover + Grass Cover
                                  rn_model_5_coye, #Treatment * Grass Cover
                                  rn_model_6_coye, #Treatment + Grass Cover
                                  rn_model_7_coye, #Treatment * Shrub Cover
                                  rn_model_8_coye, #Treatment + Shrub Cover
                                  rn_model_9_coye, #Shrub Cover * Grass Cover
                                  rn_model_10_coye, #Shrub Cover + Grass Cover
                                  rn_model_11_coye) #Null

#model names use descriptive 
model_names_coye_post_hoc <- c("Treatment + Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height", 
                               "Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height", 
                               "Treatment", 
                               "Treatment * Shrub Cover * Grass Cover", 
                               "Treatment + Shrub Cover + Grass Cover", 
...
aicc_table_coye_post_hoc <- aictab(cand.set = models_list_coye_post_hoc, modnames = model_names_coye_post_hoc)

print(aicc_table_coye_post_hoc)

# Load the master AICc table
aicc_table <- read.csv("data/aicc_table_posthoc.csv")

# Add species name to the table
aicc_table_coye_post_hoc$species_code <- "Common Yellowthroat"

# Add the AICc table to the master table
aicc_table_master <- rbind(aicc_table, aicc_table_coye_post_hoc)

# Save the AICc table to csv
write.csv(aicc_table_master, "data/aicc_table_posthoc.csv", row.names = FALSE)

#vif for top model
vif_table_state_coye <- vif(rn_model_6_coye, type = "state")
vif_table_det_coye <- vif(rn_model_6_coye, type = "det")
