...
summary(rn_model_7_blgr)

#Treatment + Shrub Cover
rn_model_8_blgr <- occuRN( 
  formula = ~ I(scale(day)^2) + scale(day) + scale(wind_speed) + scale(precipitation) 
...
#AICc comparison
models_list_blgr_post_hoc <- list(rn_model_0_blgr, #Treatment + Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height
                                  rn_model_1_blgr, #Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height
                                  rn_model_2_blgr, #Treatment
                                  rn_model_3_blgr, #Treatment * Shrub Cover * Grass Cover
                                  rn_model_4_blgr, #Treatment + Shrub Cover + Grass Cover
                                  rn_model_5_blgr, #Treatment * Grass Cover
                                  rn_model_6_blgr, #Treatment + Grass Cover
                                  rn_model_7_blgr, #Treatment * Shrub Cover
                                  rn_model_8_blgr, #Treatment + Shrub Cover
                                  rn_model_9_blgr, #Shrub Cover * Grass Cover
                                  rn_model_10_blgr, #Shrub Cover + Grass Cover
                                  rn_model_11_blgr) #Null

#model names use descriptive 
model_names_blgr_post_hoc <- c("Treatment + Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height", 
                               "Shrub Cover + Grass Cover + Forb Cover + Shrub Height + Grass Height", 
                               "Treatment", 
                               "Treatment * Shrub Cover * Grass Cover", 
                               "Treatment + Shrub Cover + Grass Cover", 
...
aicc_table_blgr_post_hoc <- aictab(cand.set = models_list_blgr_post_hoc, modnames = model_names_blgr_post_hoc)

print(aicc_table_blgr_post_hoc)

# Load the master AICc table
aicc_table <- read.csv("data/aicc_table_posthoc.csv")

# Add species name to the table
aicc_table_blgr_post_hoc$species_code <- "Blue Grosbeak"

# Add the AICc table to the master table
aicc_table_master <- rbind(aicc_table, aicc_table_blgr_post_hoc)

# Save the AICc table to csv
write.csv(aicc_table_master, "data/aicc_table_posthoc.csv", row.names = FALSE)

#vif for top model
vif_table_state_blgr <- vif(rn_model_6_blgr, type = "state")
vif_table_det_blgr <- vif(rn_model_6_blgr, type = "det")
...
gof_blgr <- mb.gof.test(rn_model_6_blgr, nsim=100, c.hat.est=TRUE, model.type="royle-nichols")
print(gof_blgr)
